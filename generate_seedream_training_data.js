/**
 * VeriSource — Seedream Training Data Generator
 * Generates photorealistic face/portrait images via Step AI API.
 * Saves directly to /mnt/verisource/training-data/ai/seedream/
 * Run on RunPod: SEEDREAM_API_KEY=your_key node generate_seedream_training_data.js
 * Get API key at: platform.stepfun.com
 */
...
const SEEDREAM_API_KEY = process.env.SEEDREAM_API_KEY || 'YOUR_SEEDREAM_API_KEY_HERE';
const OUT_DIR = process.env.OUTPUT_DIR || '/mnt/verisource/training-data/ai/seedream';
...
async function worker(id) {
  while ((done + fail) < TARGET) {
    ...
    const dest = path.join(OUT_DIR, `seedream_${imageId}.jpg`);
...
async function main() {
  if (SEEDREAM_API_KEY === 'YOUR_SEEDREAM_API_KEY_HERE') {
    console.error('❌ Set your Seedream API key: SEEDREAM_API_KEY=your_key node generate_seedream_training_data.js');
    console.error('   Get a key at: platform.stepfun.com');
    process.exit(1);
  }
  ...
  console.log(`Seedream Portrait Generator`);
