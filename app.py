"""
VeriSource GPU Forensics Service v3.1.0
========================================
...
class FrequencyClassifier(nn.Module):
    """Binary AI/real frequency classifier."""
    def __init__(self):
        super().__init__()
...
class FrequencyCNN(nn.Module):
    """Generator classifier frequency branch."""
    def __init__(self, num_classes):
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(), nn.Linear(256 * 4 * 4, 512), nn.ReLU(), nn.Dropout(0.5), nn.Linear(512, num_classes),
        )
...
class GeneratorClassifier(nn.Module):
    """EfficientNet-B2 + FrequencyCNN + CLIP ensemble for generator classification."""
    def __init__(self, num_classes, clip_dim=768):
        super().__init__()
        self.freq_cnn = FrequencyCNN(num_classes)
        self.visual_cnn = tv_models.efficientnet_b2(weights=None)
        in_features = self.visual_cnn.classifier[1].in_features
        self.visual_cnn.classifier = nn.Sequential(nn.Dropout(0.3), nn.Linear(in_features, num_classes))
        self.clip_head = nn.Sequential(
            nn.Linear(clip_dim, 512), nn.ReLU(), nn.Dropout(0.3), nn.Linear(512, num_classes),
        )
    ...
    def forward(self, images, clip_features=None):
        ...
        if clip_features is not None:
            ...
            return weights[0] * freq_logits + weights[1] * visual_logits + weights[2] * clip_logits
...
def load_clip_detector():
    try:
        ...
        classifier = nn.Sequential(nn.Linear(768, 256), nn.ReLU(), nn.Dropout(0.2), nn.Linear(256, 2)).to(DEVICE)
        trained = False
        ...
        classifier.eval()
        return {"clip_model": clip_model, "clip_preprocess": clip_preprocess, "classifier": classifier, "ready": trained}
    except ImportError:
...
def load_generator_model():
    ...
    try:
        ...
        if not class_to_idx:
            class_to_idx = checkpoint.get("labels", {})
            idx_to_class = {str(v): k for k, v in class_to_idx.items()}
        num_classes = len(class_to_idx)
        model = GeneratorClassifier(num_classes).to(DEVICE)
        model.load_state_dict(checkpoint["model_state_dict"])
        model.eval()
        val_acc = checkpoint.get("val_accuracy", checkpoint.get("val_acc", "N/A"))
        logger.info(f"✅ Generator classifier loaded ({num_classes} classes, val_acc={val_acc})")
        return {
            "ready": True,
            "model": model,
            "labels": class_to_idx,
            "label_names": {int(k): v for k, v in idx_to_class.items()},
        }
    except Exception as e:
...
app = FastAPI(title="VeriSource GPU Service", version="3.1.0", lifespan=lifespan)
    ...
    if clip_data and clip_data.get("clip_model"):
        try:
            ...
            with torch.no_grad():
                ...
                clip_score = F.softmax(logits, dim=-1)[0, 1].item()
        except Exception as e:
    ...
    if freq_data and freq_data.get("model"):
        try:
            ...
            with torch.no_grad():
                ...
                freq_score = F.softmax(logits, dim=-1)[0, 1].item()
        except Exception as e:
            logger.error(f"Frequency error: {e}")
    scores, weights = [], []
    if clip_score is not None and clip_data.get("ready"):
        scores.append(clip_score); weights.append(0.75)
    if freq_score is not None and freq_data.get("ready"):
        scores.append(freq_score); weights.append(0.25)
    if scores:
...
async def health():
    ...
    gpu_info = {}
    if torch.cuda.is_available():
        gpu_info = {"name": torch.cuda.get_device_name(0), "memory_allocated_gb": round(torch.cuda.memory_allocated(0) / 1e9, 2)}
    return {
        "status": "ok", "device": DEVICE, "gpu": gpu_info,
        "models_loaded": {"clip_detector": clip_loaded, "freq_classifier": freq_loaded, "generator_classifier": gen_ready},
        "models_trained": {"clip_detector": clip_trained, "freq_classifier": freq_trained, "generator_classifier": gen_ready},
        "ensemble": {"all_trained": clip_trained and freq_trained, "any_trained": clip_trained or freq_trained},
    }


@app.post("/analyze")
async def analyze(file: UploadFile = File(...), x_gpu_api_key: str = Header(None)):
    ...
    clip_data = models.get("clip", {}); freq_data = models.get("freq", {})
    elapsed = time.time() - start
    result = {
        "success": True, "is_ai": is_ai,
        "ai_score": round(ensemble_score, 4), "label": "ai" if is_ai else "real",
        "confidence": round(abs(ensemble_score - 0.5) * 200, 1),
        "ensemble": {"ai_probability": round(ensemble_score, 4), "method": "weighted_average",
                     "all_trained": all(models.get(m, {}).get("ready", False) for m in ["clip", "freq"])},
        "models": {}, "gpu_inference_ms": round(elapsed * 1000, 1), "device": DEVICE,
    }
    if clip_score is not None:
        result["models"]["clip"] = {"ai_probability": round(clip_score, 4), "label": "ai" if clip_score > 0.5 else "real", "trained": clip_data.get("ready", False)}
    if freq_score is not None:
        result["models"]["frequency"] = {"ai_probability": round(freq_score, 4), "label": "ai" if freq_score > 0.5 else "real", "trained": freq_data.get("ready", False)}
    if generator:
...
@app.post("/detect")
async def detect(file: UploadFile = File(...), x_gpu_api_key: str = Header(None)):
    await verify_api_key(x_gpu_api_key)
    start = time.time()
    try:
        image = Image.open(io.BytesIO(await file.read())).convert("RGB")
    ...
    elapsed = time.time() - start
    return {
        "is_ai": is_ai, "confidence": round(ensemble_score, 4), "ensemble_score": round(ensemble_score, 4),
        ...
        "likely_ai_generated": is_ai, "ai_confidence": round(ensemble_score * 100, 1),
        "inference_ms": round(elapsed * 1000, 1), "device": DEVICE,
    }


@app.post("/classify-generator")
async def classify_generator(file: UploadFile = File(...), x_gpu_api_key: str = Header(None)):
    await verify_api_key(x_gpu_api_key)
    start = time.time()
    try:
        image = Image.open(io.BytesIO(await file.read())).convert("RGB")
    ...
    return {"generator": generator, "confidence": round(gen_confidence, 4), "raw_scores": gen_scores, "inference_ms": round(elapsed * 1000, 1), "device": DEVICE}


@app.post("/analyze-batch")
async def analyze_batch(files: list[UploadFile] = File(...), x_gpu_api_key: str = Header(None)):
    await verify_api_key(x_gpu_api_key)
    start = time.time()
    frame_results = []
    for f in files:
        try:
            ...
            frame_results.append({"ai_probability": round(ensemble_score, 4), "label": "ai" if ensemble_score > 0.5 else "real"})
        except Exception as e:
            frame_results.append({"error": str(e)})
    elapsed = time.time() - start
    return {"success": True, "frame_count": len(files), "results": frame_results,
            "total_time_ms": round(elapsed * 1000, 1), "per_frame_ms": round(elapsed / max(len(files), 1) * 1000, 1), "device": DEVICE}
