 ...
 * - Optional article HTML fallback (og:image / twitter:image / JSON-LD ImageObject)
 ...
 *
 * Notes:
 * - Your prior "pHash" was an aHash-style average threshold. In v2 we keep aHash (as ahash)
 *   and implement a real pHash (DCT-based) for better robustness.
 */
...
async function initDatabase() {
  ...
  try {
    ...
    await client.query(`CREATE INDEX IF NOT EXISTS idx_news_images_sha256 ON news_images(sha256)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_news_images_ahash_bigint ON news_images(ahash_bigint)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_news_images_dhash_bigint ON news_images(dhash_bigint)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_news_images_phash_bigint ON news_images(phash_bigint)`);
